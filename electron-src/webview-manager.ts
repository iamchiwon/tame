...
export class WebViewManager {
  private webViews: Map<number, WebViewInfo> = new Map();
  ...
  public registerWebView(tabId: number, webContents: WebContents, url: string, title: string) {
    // 기존 웹뷰가 있다면 정리
    this.unregisterWebView(tabId);

    // 알림 핸들러 생성
    const notificationHandler = new NotificationHandler(webContents, tabId);
  ...
  public unregisterWebView(tabId: number) {
  ...
  public updateWebViewInfo(tabId: number, updates: Partial<Pick<WebViewInfo, 'url' | 'title'>>) {
    const webViewInfo = this.webViews.get(tabId);
    if (webViewInfo) {
  ...
  public getWebViewInfo(tabId: number): WebViewInfo | undefined {
    return this.webViews.get(tabId);
  }

  public getAllWebViews(): WebViewInfo[] {
    return Array.from(this.webViews.values());
  }

  public getWebViewByUrl(url: string): WebViewInfo | undefined {
    return Array.from(this.webViews.values()).find(webView =>
      webView.url.includes(url) || url.includes(webView.url)
    );
  }

  private setupWebViewEventListeners(tabId: number, webContents: WebContents) {
    // 페이지 제목 변경 감지
